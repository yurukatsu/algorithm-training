...
void solve()
{
    ...
    static int dp_len[2001], dp_op[2001], dp_a[2001], dp_b[2001];
    for (int i = 0; i <= N; i++)
    {
        dp_len[i] = INF;
        dp_op[i] = dp_a[i] = dp_b[i] = 0;
    }
    ...
    for (int n = 1; n <= N; n++)
    {
        ...
        {
            int j = n - i;
            if (dp_len[i] == INF || dp_len[j] == INF)
                continue;
            // i + j
            int cost = dp_len[i] + 1 + dp_len[j];
            if (cost < dp_len[n])
            {
                dp_len[n] = cost;
                dp_op[n] = 1;
        ...
        for (int i = 2; i * i <= n; i++)
        {
            ...
            if (dp_len[i] != INF && dp_len[j] != INF)
            {
                int cost = dp_len[i] + 1 + dp_len[j] + (dp_op[i] == 1 ? 2 : 0) + (dp_op[j] != 0 ? 2 : 0);
                if (cost < dp_len[n])
                {
                    dp_len[n] = cost;
                    dp_op[n] = 2;
            ...
            // j * i
            if (dp_len[j] != INF && dp_len[i] != INF)
            {
                int cost = dp_len[j] + 1 + dp_len[i] + (dp_op[j] == 1 ? 2 : 0) + (dp_op[i] != 0 ? 2 : 0);
                if (cost < dp_len[n])
                {
                    dp_len[n] = cost;
                    dp_op[n] = 2;
    ...
    // 文字列再構築
    function<string(int)> build = [&](int n) -> string
    {
        if (dp_op[n] == 0)
        {
            // literal: '1' を dp_len[n] 回
            return string(dp_len[n], '1');
        }
        int L = dp_a[n], R = dp_b[n];
        string ls = build(L), rs = build(R);
        if (dp_op[n] == 1)
        {
            // 加算はそのままチェーン可能
            return ls + "+" + rs;
